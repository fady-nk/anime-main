...
if(empty($_POST['name']) OR empty($_POST['show_id'])){
    echo "<script>alert('one or more inputs are empty')</script>";
          ...
          $dir_thumbnail = "uploads/".basename($thumbnail);
          $dir_video = "uploads/".basename($video);

          $insert = $conn->prepare("INSERT INTO episodes (name, show_id, video, thumbnail) 
          values(:name, :show_id, :video, :thumbnail)");
          $insert->execute([
             "name" => $name,
             "show_id" => $show_id,
             "thumbnail" => $thumbnail,
